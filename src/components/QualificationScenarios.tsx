...
interface Props { group: GroupName }
...
export default function QualificationScenarios({ group }: Props) {
  ...
  const scenarios = useMemo(() => {
    ...
    if (remaining.length === 0 || remaining.length >= 6) return [];
    return remaining.flatMap((rm) =>
      TEAMS.filter((t) => t.group === group).map((team) => {
        const isHome = rm.homeTeamId === team.id;
        const out = [];
        for (const [hs, as, lbl] of [[1,0,"Win"],[0,0,"Draw"],[0,1,"Lose"]] as const) {
          const sim = new Map(results);
          const id = rm.id;
          sim.set(id, isHome ? {matchId:id,homeScore:hs,awayScore:as} : {matchId:id,homeScore:as,awayScore:hs});
          const row = computeGroupStandings(group, sim).find((r) => r.teamId === team.id)!;
          out.push({label:lbl,qualifies:row.position<=2,third:row.position===3});
        }
        ...
        return (
          <div key={s.teamId + "-" + s.matchId} className="qs-row">
            ...
            <span className="qs-outcomes">
              {s.outcomes.map((o) => (
                <span key={o.label} className={"qs-badge " + (o.qualifies ? "qs-in" : o.third ? "qs-third" : "qs-out")}>
